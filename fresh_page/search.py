...
def breadth_first_graph_search(problem, display=False):
    ...
    if problem.goal_test(node.state):
        if display:
            print("0 nodes have been expanded and 0 paths remain in the frontier")
        return node
    frontier = deque([node])
    explored = set()
    expansions = 0
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            if display:
                print(f"{expansions} nodes have been expanded and {len(frontier)} paths remain in the frontier")
            return node
    ...
    if display:
        print(f"{expansions} nodes have been expanded (no solution)")
    return None
...
node = astar_search(eight_puzzle, h=None, display=True)
if node is None:
    print("No solution found by A*")
else:
    solution = node.solution()
    print(solution)
    print(f"Number of actions to solve (A*): {len(solution)}")
